...
from OGCModel.flowstep3d_util import PointNetSetAbstraction, FlowEmbedding, PointNetFeaturePropogation
...
class GRU(nn.Module):
    def __init__(self, npoint, hidden_dim, input_dim, use_instance_norm):
        ...
        self.convz = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=4, in_channel=in_ch,
                                            mlp=[hidden_dim], group_all=False, use_act=False, use_instance_norm=use_instance_norm)
        self.convr = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=4, in_channel=in_ch,
                                            mlp=[hidden_dim], group_all=False, use_act=False, use_instance_norm=use_instance_norm)
        self.convq = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=4, in_channel=in_ch,
                                            mlp=[hidden_dim], group_all=False, use_act=False, use_instance_norm=use_instance_norm)
...
class NoGRU(nn.Module):
    def __init__(self, npoint, hidden_dim, input_dim, use_instance_norm):
        ...
        self.conv = PointNetSetAbstraction(npoint=int(npoint / 4), radius=None, nsample=4, in_channel=input_dim,
                                           mlp=[hidden_dim], group_all=False, use_instance_norm=use_instance_norm)
...
class FlowStep3D(nn.Module):
    def __init__(self, npoint=2048, use_instance_norm=False, loc_flow_nn=8, loc_flow_rad=0.1, k_decay_fact=1.0):
    ...
    def get_x_slim(self, feats1_loc_new, corr_feats):
        x = torch.cat([feats1_loc_new, corr_feats], dim=1) # [64, 128]
        return x
    ...
    def forward(self, pc1, pc2, feature1, feature2, iters=1):
        ...
        for iter in range(iters-1):
            ...
            # Recurrent update with GRU (original paper)
...
if __name__ == '__main__':
    ...
    pretrain = 'pretrain_weights/flowstep_unsup/epoch=23.ckpt'
    weights_loaded = torch.load(pretrain)['state_dict']
