...
from pointnet2.pointnet2 import grouping_operation,gather_operation, ball_query, QueryAndGroup, GroupAll, gather_nd,furthest_point_sample, knn, three_nn
...
class FlowEmbedding(nn.Module):
    ...
    def forward(self, pos1, pos2, feature1, feature2):
        ...
        B, N, C = pos1_t.shape
        if self.knn:
            dist, idx = knn(self.nsample, pos1_t, pos2_t)
            ...
            idx, cnt = ball_query(self.radius, self.nsample, pos2_t, pos1_t)
            _, idx_knn = knn(self.nsample, pos1_t, pos2_t)
...
class PointNetSetAbstraction(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, mlp, group_all,
        ...
        if group_all:
            self.queryandgroup = GroupAll(self.use_xyz)
        else:
            self.queryandgroup = QueryAndGroup(radius, nsample, self.use_xyz)
    ...
    def forward(self, xyz, points, fps_idx=None):
        ...
        if (self.group_all == False) and (self.npoint != -1):
            if fps_idx == None:
                fps_idx = furthest_point_sample(xyz_t, self.npoint)  # [B, N]
            new_xyz = gather_operation(xyz, fps_idx)  # [B, C, N]
        else:
            new_xyz = xyz
...
class PointNetFeaturePropogation(nn.Module):
    ...
    def forward(self, pos1, pos2, feature1, feature2):
        ...
        dists, idx = three_nn(pos1_t, pos2_t)
        dists[dists < 1e-10] = 1e-10
        weight = 1.0 / dists
        weight = weight / torch.sum(weight, -1, keepdim=True)  # [B,N,3]
        interpolated_feat = torch.sum(grouping_operation(feature2, idx) * weight.view(B, 1, N, 3),
                                      dim=-1)  # [B,C,N,3]
